...
def generate_launch_description():
    ...
    gazebo=ExecuteProcess(cmd=['gz','sim','-r',LaunchConfiguration('world')],output='screen',name='gazebo')
    spawn=TimerAction(period=5.0,actions=[
        LogInfo(msg='[sim_mission] Spawning AUV...'),
        ExecuteProcess(cmd=['ros2','run','ros_gz_sim','create','-world','camera_world','-file',model_sdf,'-name','auv_box','-x',LaunchConfiguration('spawn_x'),'-y',LaunchConfiguration('spawn_y'),'-z',LaunchConfiguration('spawn_z'),'-Y',LaunchConfiguration('spawn_yaw')],output='screen',name='spawn_auv')
    ])
    bridge=TimerAction(period=7.0,actions=[IncludeLaunchDescription(PythonLaunchDescriptionSource(bridge_launch))])
    stereo=TimerAction(period=9.0,actions=[IncludeLaunchDescription(PythonLaunchDescriptionSource(os.path.join(stereo_share,'launch','stereo_image_proc.launch.py')),launch_arguments={'left_namespace':'/auv/camera/left','right_namespace':'/auv/camera/right'}.items())])
    nodes=TimerAction(period=12.0,actions=[
    ...
    return LaunchDescription([world_arg,spawn_x_arg,spawn_y_arg,spawn_z_arg,spawn_yaw_arg,gazebo,spawn,bridge,stereo,nodes])
