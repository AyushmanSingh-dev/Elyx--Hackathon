...
class GateNavigatorNode(Node):
    def __init__(self):
        ...
        self.commit_tick_count = self.aligned_tick_count = self.stop_pause_tick = self.log_tick = 0
        self.done_logged = False
        self._dbg_distance = self._dbg_bearing_error = self._dbg_nav_source = 0.0
        ...
        self.get_logger().info(f'Gate Navigator started | gate={self.gate_position_topic} | odom={self.odom_topic}')
    ...
    def _body_to_odom(self, x_camera, y_camera, z_camera):
        ...
        return (self.current_x + xb*c - yb*s, self.current_y + xb*s + yb*c, self.current_z + zb)
    ...
    def _fresh(self):
        if self.latest_gate_time is None:
            return False
        return (self.get_clock().now()-self.latest_gate_time).nanoseconds/1e9 <= self.detection_timeout_s
    ...
    def gate_position_callback(self, msg: GatePosition3D):
        if not msg.detected:
            return
        x, y, z, confidence = map(float, (msg.x_fwd_m, msg.y_left_m, msg.z_up_m, msg.confidence))
    ...
    def control_loop(self):
        ...
        if self.state == 'SEARCH':
            self.commit_tick_count = self.aligned_tick_count = 0
            cmd.angular.z = 0.15
            ...
            if self.gate_center_odom is None:
                ...
                if distance <= self.standoff_reach_threshold_m:
                    self.aligned_tick_count = 0; next_state = 'FINAL_ALIGN'
                else:
                    cmd.linear.x = self.track_speed_mps
                    cmd.angular.z = max(-self.max_yaw_rate_radps, min(self.max_yaw_rate_radps, self.track_yaw_kp*bearing_error))
            ...
            if self.stop_pause_tick >= self.stop_pause_ticks and self.target_yaw is not None:
                ...
                self.last_track_x, self.last_track_y = self.current_x, self.current_y
                self.stop_pause_tick = 0; self.aligned_tick_count = 0; next_state = 'RETURN_ALIGN'
            ...
            if self.target_yaw is None:
                ...
                if abs(err) <= self.align_threshold_rad:
                    self.aligned_tick_count += 1
                    if self.aligned_tick_count >= self.align_ticks_required:
                        self.aligned_tick_count = 0; self.last_track_x, self.last_track_y = self.current_x, self.current_y; next_state = 'RETURN_CROSS'
                else:
                    self.aligned_tick_count = 0
                    cmd.angular.z = max(-self.max_yaw_rate_radps, min(self.max_yaw_rate_radps, self.final_yaw_kp*err))
                self._dbg_distance, self._dbg_bearing_error = self.return_dist_remaining, err
