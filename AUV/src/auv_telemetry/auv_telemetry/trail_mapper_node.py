#!/usr/bin/env python3
"""AUV trail mapper for typed gate/bin perception messages."""
...
import rclpy
from auv_msgs.msg import BinPosition3D, GatePosition3D
...
class TrailMapperNode(Node):
    def __init__(self):
        ...
        self.trail, self.gates, self.bins = [], [], []
        self.current = None
        self.total_dist = 0.0
        self.last_trail = None
        self.create_subscription(Odometry, self.odom_topic, self.odom_callback, 10)
        self.create_subscription(GatePosition3D, self.gate_topic, self.gate_callback, 10)
        self.create_subscription(BinPosition3D, self.bin_topic, self.bin_callback, 10)
    ...
    def _sensor_to_odom(self, x, y, z):
        ...
        xb, yb, zb = x-self.camera_offset_fwd_m, y-self.camera_offset_left_m, z-self.camera_offset_up_m
    ...
    def odom_callback(self, msg):
        ...
        if self.last_trail is None or math.dist(self.current[:3], self.last_trail) >= self.min_trail_distance_m:
            d = 0.0 if self.last_trail is None else math.dist(self.current[:3], self.last_trail)
            self.total_dist += d
            self.last_trail = self.current[:3]
            self.trail.append(self.current[:3])
            i = len(self.trail)
            self.csv_writer.writerow([i, *self.current[:3], d, self.total_dist])
    ...
    def _add_detection(self, collection, point, confidence):
        ...
        if nearest is not None and nearest_d <= self.cluster_distance_m:
            ...
            nearest['p'] = tuple((1-a)*nearest['p'][i] + a*point[i] for i in range(3))
    ...
    def gate_callback(self, msg: GatePosition3D):
        if msg.detected:
            self._add_detection(self.gates, self._sensor_to_odom(msg.x_fwd_m, msg.y_left_m, msg.z_up_m), float(msg.confidence))

    def bin_callback(self, msg: BinPosition3D):
        if msg.detected:
            self._add_detection(self.bins, self._sensor_to_odom(msg.x_fwd_m, msg.y_left_m, msg.z_up_m), float(msg.confidence))

    def publish(self):
        path = Path(); path.header.frame_id = self.odom_frame; path.header.stamp = self.get_clock().now().to_msg()
        for x,y,z in self.trail:
            ...
            pose.pose.orientation.w = 1.0; path.poses.append(pose)
        self.path_pub.publish(path)
        markers = MarkerArray(); stamp = self.get_clock().now().to_msg()
        for i,m in enumerate(self.gates): markers.markers.append(self._gate_marker(i,m['p'],stamp))
        for i,m in enumerate(self.bins): markers.markers.append(self._bin_marker(i,m['p'],stamp))
        if markers.markers: self.marker_pub.publish(markers)

    def _gate_marker(self, i, p, stamp):
        m = Marker(); m.header.frame_id=self.odom_frame; m.header.stamp=stamp; m.ns='gate_markers'; m.id=i; m.type=Marker.CUBE; m.action=Marker.ADD
        m.pose.position.x,m.pose.position.y,m.pose.position.z=p
        h=self.gate_visual_yaw_rad/2.0; m.pose.orientation.z,m.pose.orientation.w=math.sin(h),math.cos(h)
    ...
    def _bin_marker(self, i, p, stamp):
        m=Marker(); m.header.frame_id=self.odom_frame; m.header.stamp=stamp; m.ns='bin_markers'; m.id=i; m.type=Marker.SPHERE; m.action=Marker.ADD
    ...
    def destroy_node(self):
        try: self._export_maps()
        except Exception as exc: self.get_logger().error(f'Map export failed: {exc}')
    ...
    def _export_maps(self):
        ...
        if self.trail:
            xs,ys=zip(*[(p[0],p[1]) for p in self.trail]); ax.plot(xs,ys,'-',label='AUV trail'); ax.plot(xs[0],ys[0],'o',label='Start'); ax.plot(xs[-1],ys[-1],'s',label='End')
        if self.gates: ax.scatter([m['p'][0] for m in self.gates],[m['p'][1] for m in self.gates],marker='s',s=180,facecolors='none',linewidths=2.5,label='Gate')
        if self.bins: ax.scatter([m['p'][0] for m in self.bins],[m['p'][1] for m in self.bins],marker='o',s=150,facecolors='none',linewidths=2.5,label='Bin')
        ...
        if self.create_3d_html and PLOTLY_AVAILABLE:
            fig=go.Figure()
            if self.trail: fig.add_trace(go.Scatter3d(x=[p[0] for p in self.trail],y=[p[1] for p in self.trail],z=[p[2] for p in self.trail],mode='lines',name='AUV trail'))
            if self.gates: fig.add_trace(go.Scatter3d(x=[m['p'][0] for m in self.gates],y=[m['p'][1] for m in self.gates],z=[m['p'][2] for m in self.gates],mode='markers',name='Gate'))
            if self.bins: fig.add_trace(go.Scatter3d(x=[m['p'][0] for m in self.bins],y=[m['p'][1] for m in self.bins],z=[m['p'][2] for m in self.bins],mode='markers',name='Bin'))
            fig.update_layout(title='AUV Mission Map — Interactive 3D',scene=dict(xaxis_title='X (m)',yaxis_title='Y (m)',zaxis_title='Z (m)',aspectmode='data')); fig.write_html(self.html_path)


def main(args=None):
    rclpy.init(args=args); node=TrailMapperNode()
    try: rclpy.spin(node)
