...
YOLO class 1 represents the complete bin as one object. This node publishes
only the highest-confidence valid bin and never assumes individual parts.
"""
...
class BinDetectorNode(Node):
    def __init__(self):
        ...
        self.model_relative_path = self.declare_parameter('model_relative_path', 'weights/gate_detector_v2.pt').value
        ...
        if self.debug_enabled:
            try:
                ...
                self.get_logger().error(f'Could not create debug directory: {exc}')
    ...
    def image_callback(self, msg: Image):
        try:
            ...
            self._log_throttled('error', f'Failed to convert camera image: {exc}')
        ...
        try:
            results = self.model.predict(source=cv_image, verbose=False, conf=self.confidence_threshold,
                                         iou=self.iou_threshold, imgsz=self.image_size, device=self.device)
        except Exception as exc:
            self._log_throttled('error', f'YOLO inference failed: {exc}')
            self._publish_no_detection()
            return
        if not results:
            self._publish_no_detection()
        ...
        if self.debug_enabled:
            try:
                cv2.imwrite(os.path.join(self.debug_directory, 'latest_bin_detection.jpg'), result.plot())
            except Exception as exc:
                self._log_throttled('warn', f'Failed to save bin debug image: {exc}')
        ...
        if boxes is not None:
            for box in boxes:
                try:
                    ...
                    if class_id not in self.bin_class_ids or confidence < self.confidence_threshold:
                        continue
        ...
        self._log_throttled('info', f'Bin detected | center=({cx:.1f},{cy:.1f}) | bbox={width:.0f}x{height:.0f} | conf={confidence:.2f}')
