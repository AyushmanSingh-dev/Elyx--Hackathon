#!/usr/bin/env python3
"""Production YOLO gate detector.

YOLO class 0 represents the complete gate as one object. The detector publishes
only the highest-confidence gate and never treats poles as separate objects.
"""
...
class GateDetectorNode(Node):
    def __init__(self):
        ...
        if not self.gate_class_ids:
            raise ValueError('gate_class_ids cannot be empty')
    ...
    def image_callback(self, msg: Image):
        ...
        if self.debug_enabled:
            try:
                ...
                self._log_throttled('warn', f'Failed to save debug image: {exc}')
        ...
        if boxes is not None:
            for box in boxes:
                try:
                    ...
                    if class_id not in self.gate_class_ids or confidence < self.confidence_threshold:
                        continue
