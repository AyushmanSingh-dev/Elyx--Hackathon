...
class GateDetectorNode(Node):
    def __init__(self):
        ...
        if os.path.isfile(self.weights_path):
            try:
                ...
                self.get_logger().error(
                    f'Could not load YOLO weights; detector will idle: {exc}'
                )
        else:
            self.get_logger().warn(
                f'YOLO weights not found: {self.weights_path}. '
                'Simulation will run with perception idle.'
            )
        ...
        if self.debug_enabled:
            try:
                ...
                self.get_logger().warn(f'Could not create debug directory: {exc}')
    ...
    def image_callback(self, msg):
        ...
        if self.debug_enabled:
            try:
                cv2.imwrite(
                    os.path.join(self.debug_directory, 'latest_gate_detection.jpg'),
                    result.plot(),
                )
            except Exception:
                pass
        ...
        if boxes is not None:
            for box in boxes:
                try:
                    ...
                    if cid not in self.gate_class_ids or conf < self.confidence_threshold:
                        continue
        ...
        self._log('info', f'Gate detected | center=({cx:.1f},{cy:.1f}) | conf={conf:.2f}')
