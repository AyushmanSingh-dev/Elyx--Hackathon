...
centre and robust stereo disparity sampling around that centre.
"""
...
import numpy as np
import rclpy
from auv_msgs.msg import GateDetection2D, GatePosition3D
...
class GateLocalizerNode(Node):
    def __init__(self):
        ...
        self.disparity_topic = self.declare_parameter('disparity_topic', '/auv/camera/left/disparity').value
        self.camera_info_topic = self.declare_parameter('camera_info_topic', '/auv/camera/left/camera_info').value
        ...
        self.pos_pub = self.create_publisher(GatePosition3D, self.output_topic, 10)
        self.create_subscription(GateDetection2D, self.detection_topic, self.detection_callback, 10)
        ...
        self.ts = message_filters.ApproximateTimeSynchronizer(
            [disp_sub, info_sub], queue_size=10, slop=0.10
        )
    ...
    def detection_callback(self, msg: GateDetection2D):
        if not msg.detected:
            self.latest_detection = None
            self.latest_detection_time = None
            return
    ...
    def sync_callback(self, disp_msg, info_msg):
        ...
        try:
            disparity = np.asarray(
                self.bridge.imgmsg_to_cv2(disp_msg.image, desired_encoding='passthrough')
            )
        ...
        out.gate_yaw_body_rad = float('nan')
